...
uploaded_file = st.file_uploader(
    ...
    help="Maximum file size is 30 MB."
)
...
if uploaded_file is not None:
    ...
    if file_size_mb > MAX_FILE_SIZE_MB:
        st.error(
            ...
            f"Please select an audio/video file under **30 MB**."
        )
        st.stop()
    ...
    if start_clicked:
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            try:
                ...
                try:
                    log("⚡ Compressing audio stream to 32kbps Mono MP3 (< 3 MB)...")
                    flush_logs()
                    ...
                    if os.path.exists(compressed_path) and os.path.getsize(compressed_path) > 0:
                        ...
                        log(f"⚡ Compression complete! Reduced file size to {c_mb:.1f} MB.")
